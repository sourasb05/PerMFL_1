...
class User:
    ...
    def __init__(self, device, id, train_data, test_data, model, batch_size=0, alpha=0, beta=0, eta=0, lamda=0, gamma=0,
                 local_epochs=0):
    ...
    def test_thetabar(self, theta_bar):
        self.model.eval()
        test_acc = 0
        # self.personalized_team_model = copy.deepcopy(list(team_update))
        self.update_parameters(theta_bar)
        for x, y in self.testloaderfull:
            x, y = x.to(self.device), y.to(self.device)
            output = self.model(x)
            test_acc += (torch.sum(torch.argmax(output, dim=1) == y)).item()
